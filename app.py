...
def classify_and_extract(note):
    ...
    match = re.search(r'(Business Application Request|tkt|sr|inc|ticket|مرجعي|incident|اس ار|انسدنت)[\s\S]{0,50}?(\d{4,})', note_lower)
        
    if match:
        ticket_num = int(match.group(2))
        ticket_keyword = match.group(1).lower()

        sr_keywords = ['sr', 'مرجعي', 'اس ار','Business Application Request']
